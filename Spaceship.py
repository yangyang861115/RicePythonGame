#http://www.codeskulptor.org/#user38_MzLztCqnL9_9.py
...
class Ship:
    ...
    def shoot(self):       
        global a_missile
        forward = angle_to_vector(self.angle)
        a_missile = Sprite([self.pos[0] + forward[0] * self.radius, self.pos[1] + forward[1] * self.radius], 
                           [self.vel[0] + 1.5 * forward[0], self.vel[1] + 1.5 * forward[1]], 
...
def draw(canvas):
    ...
    canvas.draw_text(str(lives), (40, 80), 30, 'While')
    canvas.draw_text('Score', (700, 40), 30, 'White')
    canvas.draw_text(str(score), (700, 80), 30, 'While')
...
def rock_spawner():
    ...
    a_rock = Sprite([WIDTH * random.random(), HEIGHT * random.random()], 
                    ...
                    0, (random.random() - 0.5)*0.3, asteroid_image, asteroid_info)
